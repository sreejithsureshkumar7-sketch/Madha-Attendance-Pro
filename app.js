...
async function loadAttendanceReport(){const tbody=document.getElementById("attendanceReport");if(!tbody)return;const f=getFilters();tbody.innerHTML="";let present=0,absent=0,total=0;const snap=await db.collection("attendance").get();snap.forEach(doc=>{const a=doc.data();if((!f.department||a.department===f.department)&&(!f.year||a.year===f.year)){total++;if(a.status==="Present")present++;if(a.status==="Absent")absent++;tbody.innerHTML+=`<tr><td>${a.date||""}</td><td>${a.name||""}</td><td>${a.regno||""}</td><td>${a.department||""}</td><td>${a.year||""}</td><td>${a.status||""}</td><td>${a.markedBy||""}</td></tr>`;}});document.getElementById("presentCount").innerText=present;document.getElementById("absentCount").innerText=absent;document.getElementById("totalCount").innerText=total;
                                     let percentage = total > 0 ? ( (present / total)* 100).toFixed(2) : 0:
                                     document.getElementById("attendancePercentage").innerTex = percentage + "%";}
...
function downloadExcel(){
  let table = document.querySelector("#attendanceReport").closest("table").outerHTML;
